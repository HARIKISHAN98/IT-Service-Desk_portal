...
const Login = () => {
    ...
    const location = useLocation();  //initialised the location hook
    const [infoMessage, setInfoMessage] = useState(location?.state?.info || null);
    ...
    return (
        <div className='min-h-screen bg-slate-900 flex justify-center items-center p-4'>
            <div className='max-w-md w-full'>
                ...
                <div>
                    {/* Form ke upar alert block */}
                    {infoMessage && (
                        <div className="p-3 mb-4 bg-emerald-500/10 border border-emerald-500/30 rounded-lg text-emerald-400 text-xs">
                            {infoMessage}
                        </div>
                    )}
